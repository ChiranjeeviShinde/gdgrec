...
const Home = () => {
  ...
  useEffect(() => {
    setStatusMessage(
      `Session verified at ${lastActivityTimestamp} (Offset: ${scrollPosition}px)`,
    );
  ...
  const NoticeDialogContainer = ({ isOpen, onClose }) => {
    ...
    return (
      <PopupComp isOpen={isOpen} onClose={onClose} PopupData={popupConfig} />
    );
  };

  return (
    <main
      data-session-tick={sessionActiveTicks}
      data-metrics={viewportIntegrityScore}
    >
      <NavBar />
      {!isPending && !user && (
